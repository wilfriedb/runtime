...
namespace System.Xml.Serialization
{
    ...
    public class XmlCodeExporter : CodeExporter
    {
        ...
        public void ExportTypeMapping(XmlTypeMapping xmlTypeMapping)
        {
            ...
            if (xmlTypeMapping.Accessor.Any) throw new InvalidOperationException(SR.GetResourceString(SR.XmlIllegalWildcard));
        ...
        private void ExportType(TypeMapping? mapping, string? name, string? ns, ElementAccessor? rootElement, bool checkReference)
        {
            ...
            if (ExportedMappings[mapping] is null)
            {
                ExportedMappings.Add(mapping, mapping);
                if (mapping.TypeDesc is not null && mapping.TypeDesc.IsMappedType)
                {
        ...
        {
            initExpression = null;
            CodeAttributeArgument[] arguments = Array.Empty<CodeAttributeArgument>();
            ...
            if (mapping is EnumMapping)
            {
                ...
                if (value.GetType() != typeof(string)) throw new InvalidOperationException(SR.GetResourceString(SR.XmlInternalErrorDetails, "Invalid enumeration type " + value.GetType().Name));
#endif
        ...
        private object? ImportDefault(TypeMapping? mapping, string? defaultValue)
        {
            ...
            if (mapping is not null && mapping.IsList)
            {
        ...
        private object? ImportDefaultValue(TypeMapping? mapping, string defaultValue)
        {
            ...
            if (mapping is EnumMapping em && em.Constants is not null)
            {
                ...
                throw new InvalidOperationException(SR.GetResourceString(SR.XmlInvalidDefaultValue, defaultValue));
            }

            // Primitive mapping
            PrimitiveMapping pm = (PrimitiveMapping)mapping;

            if (pm.TypeDesc is not null)
            {
                ...
                else
                {
                    ...
                    {
                        return XmlCustomFormatter.ToDefaultValue(defaultValue, pm.TypeDesc.FormatterName ?? string.Empty);
                    }
                }
            }
            return DBNull.Value;
        ...
        private void AddDefaultValueAttribute(CodeMemberField? field, CodeAttributeDeclarationCollection metadata, object? defaultValue, TypeMapping? mapping, CodeCommentStatementCollection? comments, TypeDesc? memberTypeDesc, Accessor accessor, CodeConstructor? ctor)
        {
            ...
            if (memberTypeDesc is not null && !memberTypeDesc.HasDefaultSupport)
            {
            ...
            if (memberTypeDesc is not null && memberTypeDesc.IsArrayLike && accessor is ElementAccessor)
            {
                if (comments != null && defaultValue is string)
                {
                    DropDefaultAttribute(accessor, comments, memberTypeDesc.FullName);
            ...
            if (mapping?.TypeDesc is not null && mapping.TypeDesc.IsMappedType && field != null && defaultValue is string)
            {
            ...
            if (comments != null && pm.TypeDesc is not null && !pm.TypeDesc.HasDefaultSupport && pm.TypeDesc.IsMappedType)
            {
                // do not generate intializers for the user prefered types if they do not have default capability
...
#if DEBUG
                // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                if (value.GetType() != typeof(object[])) throw new InvalidOperationException(SR.GetResourceString(SR.XmlInternalErrorDetails, "Default value for list should be object[], not " + value.GetType().Name));
#endif
            ...
            if (arguments != null && pm.TypeDesc is not null && pm.TypeDesc.HasDefaultSupport && accessor.IsOptional && !accessor.IsFixed)
            {
        ...
        private CodeTypeDeclaration? ExportStruct(StructMapping mapping)
        {
            ...
            codeClass.Comments.Add(new CodeCommentStatement(SR.GetResourceString(SR.XmlRemarks), true));
            ...
            if (mapping.Members is not null)
            {
        ...
        public void AddMappingMetadata(CodeAttributeDeclarationCollection metadata, XmlTypeMapping mapping, string ns)
        {
            ...
            if (mapping.Mapping is null || mapping.Mapping is StructMapping || mapping.Mapping is EnumMapping) return;
        ...
        private void ExportArrayElements(CodeAttributeDeclarationCollection metadata, ArrayMapping? array, string? ns, TypeDesc? elementTypeDesc, int nestingLevel)
        {
            ...
            for (int i = 0; i < array.Elements.Length; i++)
            {
                ...
                if (elementMapping?.TypeDesc is null)
                {
                    return;
                }
                string elementName = Accessor.UnescapeName(arrayElement.Name);
                bool sameName = elementMapping.TypeDesc.IsArray ? false : elementName == elementMapping.TypeName;
        ...
        private void AddMemberMetadata(CodeMemberField? field, CodeAttributeDeclarationCollection metadata, MemberMapping member, string? ns, bool forceUseMemberName, CodeCommentStatementCollection? comments, CodeConstructor? ctor)
        {
            ...
            else if (member.Attribute != null)
            {
                ...
                else
                {
                    ...
                    bool sameType = mapping?.TypeDesc == member.TypeDesc ||
                        (member.TypeDesc?.IsArrayLike ?? false && mapping.TypeDesc == member.TypeDesc.ArrayElementTypeDesc);
            ...
            else
            {
                if (member.Text != null)
                {
                    ...
                    bool sameType = mapping?.TypeDesc == member.TypeDesc ||
                        (member.TypeDesc?.IsArrayLike ?? false && mapping.TypeDesc == member.TypeDesc.ArrayElementTypeDesc);
                    ExportText(metadata, sameType ? null : mapping?.TypeDesc, mapping?.TypeDesc?.IsAmbiguousDataType ?? false ? mapping?.TypeDesc?.DataType?.Name : null);
                }
                if (member.Elements?.Length == 1)
                {
                    ...
                    else
                    {
                        bool sameType = mapping?.TypeDesc == member.TypeDesc ||
                            (member.TypeDesc?.IsArrayLike ?? false && mapping.TypeDesc == member.TypeDesc.ArrayElementTypeDesc);
                        if (member.TypeDesc?.IsArrayLike ?? false)
                            sameName = false;
                ...
                {
                    if (member.Elements is not null)
                    {
        ...
        private void ExportMember(CodeTypeDeclaration codeClass, MemberMapping member, string? ns, CodeConstructor ctor)
        {
            ...
            field.Comments.Add(new CodeCommentStatement(SR.GetResourceString(SR.XmlRemarks), true));
        ...
        private void ExportProperty(CodeTypeDeclaration codeClass, MemberMapping member, string? ns, CodeIdentifiers memberScope, CodeConstructor ctor)
        {
            ...
            prop.Comments.Add(new CodeCommentStatement(SR.GetResourceString(SR.XmlRemarks), true));
